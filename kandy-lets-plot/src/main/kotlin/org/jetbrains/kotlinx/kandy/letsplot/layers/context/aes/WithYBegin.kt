...
public interface WithYBegin : BindingContext {
     ...
     * @return a [PositionalMapping] object representing the mapping.
     */
    public fun <T> yBegin(column: String): PositionalMapping<T> {
    ...
    public fun <T> yBegin(values: Iterable<T>): PositionalMapping<T> {
        return addPositionalMapping<T>(Y_BEGIN, values.toList(), null, null)
    }
    ...
    public fun <T> yBegin(values: DataColumn<T>): PositionalMapping<T> {
        return addPositionalMapping<T>(Y_BEGIN, values, null)
    }
}