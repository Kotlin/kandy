...
public interface WithYMax : WithAes {
     ...
     */
    public val yMax: org.jetbrains.kotlinx.kandy.letsplot.layers.builders.aes.ConstantSetter
    ...
    public fun <T> yMax(column: ColumnReference<T>): PositionalMapping<T> {
        return bindingHandler.addPositionalMapping<T>(Y_MAX, column.name(), null)
    }
    ...
    public fun yMax(column: String): PositionalMapping<Any?> {
        return bindingHandler.addPositionalMapping(Y_MAX, column, null)
    }
    ...
    public fun <T> yMax(values: Iterable<T>): PositionalMapping<T> {
        return bindingHandler.addPositionalMapping<T>(Y_MAX, values.toList(), null, null)
    }
    ...
    public fun <T> yMax(values: DataColumn<T>): PositionalMapping<T> {
        return bindingHandler.addPositionalMapping<T>(Y_MAX, values, null)
    }
}