...
import org.jetbrains.kotlinx.kandy.echarts.features.label.LabelFeature
...
internal fun Map<FeatureName, LayerFeature>.getLabel(): Label? {
    return (this[LabelFeature.FEATURE_NAME] as? LabelFeature)?.let {
        Label(
            ...
            textBorderColor = it.textStyle?.border?.color?.toEchartsColor(),
            textBorderWidth = it.textStyle?.border?.width,
            textBorderType = it.textStyle?.border?.type?.value,
